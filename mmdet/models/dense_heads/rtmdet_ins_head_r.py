...
class Instancekerneldistinguishmodule(BaseModule):
    ...
    def __init__(
        ...
        convs = []
        for _ in range(stacked_regtoker_convs):
            convs.append(
                ConvModule(
                    reg_inchannels,
                    in_channels,
    ...
    def forward(self, original_input, input_from_regression) -> Tensor:
        ...
        return reweight_feature 

@MODELS.register_module()
class RTMDetInsHead_R(RTMDetHead):
...
@MODELS.register_module()
class RTMDetInsSepBNHead_R(RTMDetInsHead_R):
    ...
    def _init_layers(self) -> None:
        ...
        # self.kernel_convs = nn.ModuleList()
        self.instacekerneldistinguishmodule = nn.ModuleList()
        ...
        for n in range(len(self.prior_generator.strides)):
            ...
            instacekerneldistinguish = nn.ModuleList()
            instacekerneldistinguish.append(
                    Instancekerneldistinguishmodule(
                        in_channels=self.in_channels,
                        out_channels=self.feat_channels,
                        reg_inchannels=self.feat_channels,
                        norm_cfg=self.norm_cfg,
                        act_cfg=self.act_cfg))
            for i in range(self.stacked_convs):
                chn = self.in_channels if i == 0 else self.feat_channels
            ...
            self.instacekerneldistinguishmodule.append(instacekerneldistinguish)
            self.rtm_cls.append(
                nn.Conv2d(
                    self.feat_channels,
    ...
    def forward(self, feats: Tuple[Tensor, ...]) -> tuple:
        ...
        for idx, (x, stride) in enumerate(
            ...
            for oneinstancekerneldistinguish in self.instacekerneldistinguishmodule[idx]:
                kernel_feat = oneinstancekerneldistinguish(kernel_feat, reg_feat)
